...
def sliced_score_estimation_vr(score_net, samples, n_particles=1):
    ...
    """
    '''
    used in training by authors v^T * grad(s * v) + 0.5 * s^2
    '''
    dup_samples = samples.unsqueeze(0).expand(n_particles, *samples.shape).contiguous().view(-1, *samples.shape[1:]) # expand to sampling dims
    dup_samples.requires_grad_(True) # make differentiable
    vectors = torch.randn_like(dup_samples) # sample vectors

    grad1 = score_net(dup_samples) # just basic network output s_\theta(x)
    gradv = torch.sum(grad1 * vectors) # s * v
    grad2 = autograd.grad(gradv, dup_samples, create_graph=True)[0] # grad(s*v)

    grad1 = grad1.view(dup_samples.shape[0], -1)
    loss1 = torch.sum(grad1 * grad1, dim=-1) / 2. # 0.5 * s^2 without slicing

    loss2 = torch.sum((vectors * grad2).view(dup_samples.shape[0], -1), dim=-1) # v * grad(s*v)
